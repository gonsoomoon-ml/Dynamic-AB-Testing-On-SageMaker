from algorithm import EpsilonGreedy, UCB1, ThompsonSampling, ThompsonSamplingDebug, WeightedSampling


def test_epsilon_greedy(trial):
    algo = EpsilonGreedy(
        [
            {
                "variant_name": "v1",
    ...
    lst = [algo.select_variant() for i in range(trial)]
...
def test_thompson_sampling(trial):
...
def test_thompson_sampling_w_variants(variant_list, trial, verbose=False):
    if verbose:
        algo = ThompsonSamplingDebug(variant_list)
    else:
        algo = ThompsonSampling(variant_list)
